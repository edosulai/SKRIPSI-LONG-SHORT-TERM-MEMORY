    ...
    def receive(self, text_data):
        try:
            ...
            def on_epoch_end(epoch, logs=None):
                config.valueset['epochs'].append(logs["loss"])
                self.send(text_data=json.dumps({
                    'message': f'End Epoch {epoch + 1}/{config.max_epoch} of training - loss: {round(logs["loss"], 5)}\n'
                }))
            ...
            for i in range(0, config.num_predict):
                config.logs.append(progress_bar(
                    i + 1,
                    config.num_predict,
